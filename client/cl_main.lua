    ...
    if result then
        ...
        for k, v in pairs(result) do
            storagesMenu[#storagesMenu+1] = {
                ...
                txt = "Size: "..(v.storage_size/1000).."| Owner: "..v.citizenid,
                params = {
        ...
        QBCore.Functions.Notify("NOTHING OWNED HERE!", "error")
    end
end)
...
AddEventHandler('re2-vault:createStorage', function(location)
    ...
    if cpdialog ~= nil then
        ...
        if result then
            ...
            QBCore.Functions.Notify("SUCCESS: CREATED", "success")
        else
            QBCore.Functions.Notify("ERROR: CHOOSE NEW NAME!", "error")
        end
        -- return cb(result)
    else
        QBCore.Functions.Notify("ERROR: NEED BANK MONEYS!", "error")
    end
    end
...
AddEventHandler('re2-vault:createStorageFor', function(location)
    ...
    if cpdialog ~= nil then
        ...
        if result then
            ...
            QBCore.Functions.Notify("USE NEW NAME!", "error")
        end
        -- return cb(result)
    else
        QBCore.Functions.Notify("NEED BANK MONEYS!", "error")
    end
    end
...
AddEventHandler('re2-vault:addMemberToStorage', function(data)
    ...
    if mdialog ~= nil then
        ...
        p = nil
        if result then
            QBCore.Functions.Notify("USER ADDED", "success")
        else
...
AddEventHandler('re2-vault:removeMemberFromStorage', function(data)
    ...
    if mdialog ~= nil then
        ...
        p = nil
        if result then
            QBCore.Functions.Notify("USER REMOVED", "success")
        else
            QBCore.Functions.Notify("ERROR!", "error")
        end
    end
...
AddEventHandler('re2-vault:openInventory', function(data)
        ...
        QBCore.Functions.Notify("ACCESS GRANTED", "success")
...
AddEventHandler('re2-vault:addCapacity', function(data)
    ...
    if player.money['cash'] >= tonumber(Config.StorageAddPrice) then
        ...
        if result then
            ...
            QBCore.Functions.Notify("SUCCESS: INCREASED STORAGE", "success")
        else
            QBCore.Functions.Notify("ERROR", "error")
        end
    else
        QBCore.Functions.Notify("NO BANK MONEYS!", "error")
    end
end)
...
AddEventHandler('re2-vault:changePassword', function(data)
    ...
    if mdialog ~= nil then
        ...
        p = nil
        if result then
            QBCore.Functions.Notify("SUCCESS: PASSWORD CHANGED", "success")
        else
            QBCore.Functions.Notify("ERROR", "error")
        end
    end
